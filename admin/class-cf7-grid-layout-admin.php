...
class Cf7_Grid_Layout_Admin {
	...
	*/
	...
	public function enqueue_styles( $page ) {
		...
		switch ( $screen->base ) {
			case 'post':
				...
				/** NB @since 4.0 enable dark theme */
				...
				break;
			case 'edit': // table.
				wp_enqueue_style( $this->plugin_name, $plugin_dir . 'admin/css/cf7sg-admin.css', array(), $this->version, 'all' );
				/** NB @since 4.4.0 Enabled user style enqueue */
	...
	public function enqueue_scripts( $page ) {
		...
		if ( 'plugins.php' === $page ) {
			/** NB @since 4.1.0 */
			wp_enqueue_script( 'cf7sg-plugin-update', $plugin_dir . 'admin/js/cf7sg-plugin-update.js', array( 'jquery' ), $this->version, true );
		...
		switch ( $screen->base ) {
			case 'post':
				...
				// codemirror script using v5.32.
				$codemirror_v = '5.32';
				wp_register_script( 'cf7-codemirror-js', $plugin_dir . 'assets/codemirror/codemirror.js', null, $codemirror_v, true );
				/** NB @since 3.1.2 initialise codemirror after library load and parse as attribute to anonymous functtion in cf7sg-codemirror.js */
				...
				// matchtags.
				wp_enqueue_script( 'codemirror-closetag-js', $plugin_dir . 'assets/codemirror/addon/edit/closetag.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				// fold code.
				wp_enqueue_script( 'codemirror-foldcode-js', $plugin_dir . 'assets/codemirror/addon/fold/foldcode.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-foldgutter-js', $plugin_dir . 'assets/codemirror/addon/fold/foldgutter.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-indent-fold-js', $plugin_dir . 'assets/codemirror/addon/fold/indent-fold.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-xml-fold-js', $plugin_dir . 'assets/codemirror/addon/fold/xml-fold.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-matchtag-js', $plugin_dir . 'assets/codemirror/addon/edit/matchtags.js', array( 'cf7-codemirror-js', 'codemirror-xml-fold-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-brace-fold-js', $plugin_dir . 'assets/codemirror/addon/fold/brace-fold.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-comment-fold-js', $plugin_dir . 'assets/codemirror/addon/fold/comment-fold.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-mixed-js', $plugin_dir . 'assets/codemirror/mode/htmlmixed/htmlmixed.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-javascript-js', $plugin_dir . 'assets/codemirror/mode/javascript/javascript.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-xml-js', $plugin_dir . 'assets/codemirror/mode/xml/xml.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-css-js', $plugin_dir . 'assets/codemirror/mode/css/css.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				// overlay for shortcode highligh.
				wp_enqueue_script( 'codemirror-overlay-js', $plugin_dir . 'assets/codemirror/addon/mode/overlay.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				/** NB @since 1.3.0 enable search codemirror */
				// overlay for shortcode highligh.
				wp_enqueue_script( 'codemirror-search-js', $plugin_dir . 'assets/codemirror/addon/search/search.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-jumptoline-js', $plugin_dir . 'assets/codemirror/addon/search/jump-to-line.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-matchesonscrollbar-js', $plugin_dir . 'assets/codemirror/addon/search/matchesonscrollbar.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-searchcursor-js', $plugin_dir . 'assets/codemirror/addon/search/searchcursor.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-annotatescrollbar-js', $plugin_dir . 'assets/codemirror/addon/scroll/annotatescrollbar.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );
				wp_enqueue_script( 'codemirror-dialog-js', $plugin_dir . 'assets/codemirror/addon/dialog/dialog.js', array( 'cf7-codemirror-js' ), $codemirror_v, true );

				// js beautify.
				wp_enqueue_script( 'beautify-js', $plugin_dir . 'assets/beautify/beautify.js', array( 'jquery' ), $this->version, true );
				wp_enqueue_script( 'beautify-html-js', $plugin_dir . 'assets/beautify/beautify-html.js', array( 'beautify-js' ), $this->version, true );
				...
				wp_enqueue_script( 'cf7sg-codemirror-js', $plugin_dir . "admin/js/cf7sg-codemirror{$ver}.js", array( 'jquery', 'jquery-ui-tabs', 'cf7-codemirror-js' ), $this->version, true );
				wp_localize_script(
					'cf7sg-codemirror-js',
					'cf7sgeditor',
					array(
				...
				wp_enqueue_script( $this->plugin_name, $plugin_dir . "admin/js/cf7sg-admin{$ver}.js", array( 'cf7sg-codemirror-js', 'jquery-ui-sortable' ), $this->version, true );
				/** NB @since 4.11.0 abstract out dynamic lists */
				do_action( 'cf7sg_register_dynamic_lists' );
				$lists = cf7sg_get_dynamic_lists();
				/** NB @since 5.0 get a list of usable tags */
				$tags = array();
				...
				wp_enqueue_script( 'cf7sg-benchmark-tag-js', $plugin_dir . 'admin/js/cf7sg-benchmark-tag.js', array( 'jquery', 'wpcf7-admin-taggenerator' ), $this->version, true );
				/** NB @since 3.2.0 */
				wp_enqueue_script( 'cf7sg-mail-tag-js', $plugin_dir . 'admin/js/cf7sg-mail-tag-helper.js', array( 'jquery', 'jquery-clibboard' ), $this->version, true );
				wp_localize_script(
					'cf7sg-mail-tag-js',
					'mailTagHelper',
					array(
						'msg'    => __( 'Click to copy!', 'cf7-grid-layout' ),
						/* translators: %s is a [mail_tag] shortcode */
						'filter' => __( 'Filter mailTag %s', 'cf7-grid-layout' ),
					)
				);
				/** NB @since 3.3.0 enqueue ui-grid helpers js */
				wp_enqueue_script( 'ui-grid-helpers-js', $plugin_dir . 'admin/js/cf7sg-ui-custom-helper.js', array( 'jquery' ), $this->version, true );
				do_action( 'cf7sg_enqueue_admin_editor_scripts' );

				break;
			case 'edit':
				/** NB @since 4.4.0 Enabled user script enqueue */
	...
	public function enable_warning_on_plugin_update( $plugin_file, $plugin_data, $status ) {
		...
		version_compare( CF7SG_MAJOR_UPDATE_VERSION, $plugin_data['new_version'], '<=' ) ) {
			...
			$msg = sprintf( __( 'WARNING: Read v%s update details!', 'cf7-grid-layout' ), CF7SG_MAJOR_UPDATE_VERSION );
			printf( '<style>#cf7-grid-layout-update{--rpwc2-update-msg:"%1$s";}#cf7-grid-layout-update .update-link::after {display:ruby;}</style>', esc_html( $msg ) );
		}
	}
	/**
	...
	public function modify_cf7_post_type_args( $args, $post_type ) {
		if ( $post_type === $this->cf7_post_type() ) {
			/** NB @since 2.8.3 add wpcf7_type (registered in assets/cf7-table.php) taxonomy to cf7 post type */
	 ...
	 *
	 * @since 1.0.0
	 * @param Array $taxonomy_array array of taxonomy properties to register.
	 **/
	...
	public function save_factory_metas( $cf7_form ) {
		...
		$form_classes   = array();
		foreach ( $tags as $tag ) {
			if ( in_array( $tag->basetype, $dynamic_tags, true ) ) {
		...
		if ( isset( $submits['cf72post_dynamic_select_taxonomies'] ) ) {
			$created_taxonomies = array();
			$taxonomies         = json_decode( $submits['cf72post_dynamic_select_taxonomies'], true );
			if ( empty( $taxonomies ) ) {
				$taxonomies = array();
			}
			...
			// wpg_debug($post_lists,' post lists ');
	...
	public function hide_author_metabox( $hidden, $screen, $use_defaults ) {
		if ( 'wpcf7_contact_form' !== $screen->id ) {
			return $hidden;
		}

		if ( in_array( 'authordiv', $hidden, true ) ) {
			return $hidden;
		}
	...
	public function edit_page_metabox() {
		...
		if ( post_type_exists( $this->cf7_post_type() ) ) {
			...
			/** NB @since 1.1.0 add helper metabox */
			add_meta_box(
				'meta-box-cf7sg-helper',
			...
			/** NB @since 4.6.0 enable page redirect */
			add_meta_box(
	...
	public function main_editor_metabox_display( $post ) {
		...
		require_once plugin_dir_path( __FILE__ ) . 'partials/cf7sg-admin-editor-page.php';
	}
	...
	public function info_metabox_display( $post ) {
		require_once plugin_dir_path( __FILE__ ) . 'partials/cf7sg-info-metabox-display.php';
	}
	/**
	...
	public function helper_metabox_display( $post ) {
		require_once plugin_dir_path( __FILE__ ) . '/partials/cf7sg-helper-metabox-display.php';
	}
	/**
	...
	public function grid_editor_panel( $form_obj ) {
		require_once plugin_dir_path( __FILE__ ) . '/partials/cf7sg-form-editors.php';
	}
	/**
	...
	public function display_rediect_metabox( $post ) {
		require_once plugin_dir_path( __FILE__ ) . '/partials/cf7sg-redirect-metabox-display.php';
	}
	/**
	...
	public function delete_post( $post_id ) {
		if ( get_post_type( $post_id ) !== WPCF7_ContactForm::post_type ) {
			return;
		}
		// remove viewing post.
		...
		/** NB @since 4.12.5 delete custom css/js files */
		$p    = get_post( $post_id );
		$path = get_stylesheet_directory();
		if ( realpath( "$path/js/{$p->post_name}.js" ) ) {
			unlink( "$path/js/{$p->post_name}.js" );
		}
		if ( realpath( "$path/css/{$p->post_name}.css" ) ) {
			unlink( "$path/css/{$p->post_name}.css" );
		}
	}
	/**
	 ...
	 * @param String  $post_id post id.
	 * @param WP_Post $post   post object being edited/created.
	 * @param Boolean $update true if form post udpate.
	 **/
	public function save_post( $post_id, $post, $update ) {
		...
		$args['title']  = isset( $submits['post_title'] ) ? sanitize_key( $submits['post_title'], 'Contact Form', 'save' ) : null;
		$args['locale'] = isset( $submits['wpcf7-locale'] ) ? sanitize_key( $submits['wpcf7-locale'] ) : null;
		$args['form']   = '';
		$allowed_tags   = array( 'script' => array( 'type' => 1 ) ); // filtered in function below.
		$cf7_key        = $post->post_name;
		$allowed_tags   = apply_filters( 'cf7sg_kses_allowed_html', $allowed_tags, $cf7_key );
		if ( isset( $submits['wpcf7-form'] ) ) {
			$args['form'] = wp_kses( $submits['wpcf7-form'], $allowed_tags );
		}
		$args['mail']     = isset( $submits['wpcf7-mail'] ) ? wpcf7_sanitize_mail( $submits['wpcf7-mail'] ) : array();
		$args['mail_2']   = isset( $submits['wpcf7-mail-2'] ) ? wpcf7_sanitize_mail( $submits['wpcf7-mail-2'] ) : array();
		$args['messages'] = isset( $submits['wpcf7-messages'] ) ? $submits['wpcf7-messages'] : array();
		foreach ( $args['messages'] as $key => $value ) {
			$args['messages'][ $key ] = sanitize_text_field( $value );
		}
		$args['additional_settings'] = isset( $submits['wpcf7-additional-settings'] ) ? sanitize_textarea_field( $submits['wpcf7-additional-settings'] ) : '';

		// save sub-forms if any.
		$sub_forms = json_decode( $submits['cf7sg-embeded-forms'] );
		if ( empty( $sub_forms ) ) {
			$sub_forms = array();
		}
		if ( ! is_array( $sub_forms ) ) {
			$sub_forms = array( $sub_forms );
		}
		...
		$tt_fields = json_decode( sanitize_text_field( $submits['cf7sg-table-fields'] ) );
		if ( empty( $tt_fields ) ) {
			$tt_fields = array();
		}
		if ( ! is_array( $tt_fields ) ) {
			$tt_fields = array( $tt_fields );
		}
		$sanitised_table_fields = array();
		foreach ( $tt_fields as $table_fields ) {
			if ( is_object( $table_fields ) ) { /** NB @since 2.4.2 track fields in each tables. */
				...
				foreach ( $table_fields as $field ) {
					$sanitised_table_fields[ $table ][] = sanitize_text_field( $field );
				}
			} else {
				$sanitised_table_fields[] = sanitize_text_field( $table_fields );
			}
		}
		update_post_meta( $post_id, '_cf7sg_grid_table_names', $sanitised_table_fields );
		// tabs.
		$tt_fields = json_decode( sanitize_text_field( $submits['cf7sg-tabs-fields'] ) );
		if ( empty( $tt_fields ) ) {
			$tt_fields = array();
		}
		if ( ! is_array( $tt_fields ) ) {
		...
		foreach ( $tt_fields as $tab_field ) {
			if ( is_object( $tab_field ) ) {  /** NB @since 2.4.2 track fields in each tabs. */
				...
				foreach ( $tab_field as $field ) {
					$sanitised_tab_fields[ $tab ][] = sanitize_text_field( $field );
				}
			} else {
		...
		/** Track toggled fields @since 2.5*/
		$tt_fields = json_decode( sanitize_text_field( $submits['cf7sg-toggle-fields'] ) );
		if ( empty( $tt_fields ) ) {
			$tt_fields = array();
		}
		if ( ! is_array( $tt_fields ) ) {
		...
		foreach ( $tt_fields as $tgg_field ) {
			if ( is_object( $tgg_field ) ) {
				...
				foreach ( $tgg_field as $field ) {
					$sanitised_toggled_fields[ $tgg ][] = sanitize_text_field( $field );
				}
			} else {
				$sanitised_toggled_fields[] = sanitize_text_field( $tgg_field );
			}
		}
		update_post_meta( $post_id, '_cf7sg_grid_toggled_names', $sanitised_toggled_fields );
		/** NB @since 4.0.0 track tabbed toggles */
		$ttgls = json_decode( sanitize_text_field( $submits['cf7sg-tabbed-toggles'] ) );
		if ( empty( $ttgls ) ) {
			$ttgls = array();
		}
		if ( ! is_array( $ttgls ) ) {
		...
		/** NB @since 4.0.0 track grouped toggles */
		$ttgls = json_decode( sanitize_text_field( $submits['cf7sg-grouped-toggles'] ) );
		if ( empty( $ttgls ) ) {
			$ttgls = array();
		}
		if ( is_object( $ttgls ) ) {
			$ttgls = get_object_vars( $ttgls ); // convert to array.
		} else {
			$ttgls = array();
		}
		update_post_meta( $post_id, '_cf7sg_grid_grouped_toggles', $ttgls );
		// flag tab & tables for more efficient front-end display.
		$has_tabs = ( 'true' === $submits['cf7sg-has-tabs'] ) ? true : false;
		update_post_meta( $post_id, '_cf7sg_has_tabs', $has_tabs );
		$has_tables = ( 'true' === $submits['cf7sg-has-tables'] ) ? true : false;
		update_post_meta( $post_id, '_cf7sg_has_tables', $has_tables );
		$has_toggles = ( 'true' === $submits['cf7sg-has-toggles'] ) ? true : false;
		update_post_meta( $post_id, '_cf7sg_has_toggles', $has_toggles );

		/** NB @since 1.2.3 disable cf7sg styling/js for non-cf7sg forms. */
		$is_cf7sg = ( 'true' === $submits['is_cf7sg_form'] ) ? true : false;
		update_post_meta( $post_id, '_cf7sg_managed_form', $is_cf7sg );
		$ver = sanitize_text_field( $submits['cf7sg-version'] );
		...
		if ( isset( $submits['cf7sg_codemirror_theme'] ) ) {
			update_user_meta( get_current_user_id(), '_cf7sg_cm_theme', sanitize_key( $submits['cf7sg_codemirror_theme'] ) );
		}
		if ( isset( $submits['cf7sg_js_codemirror_theme'] ) ) {
			update_user_meta( get_current_user_id(), '_cf7sg_js_cm_theme', sanitize_key( $submits['cf7sg_js_codemirror_theme'] ) );
		}
		/** Save custom js/css to stylesheet dir */
		if ( 'direct' === get_filesystem_method() ) {
			...
			if ( WP_Filesystem( $creds ) ) {
				...
				if ( ! empty( $submits['cf7sg_js_file'] ) || ! empty( $submits['cf7sg_prev_js_file'] ) ) {
					...
					if ( ! $wp_filesystem->is_dir( "$path/js/" ) ) {
						$wp_filesystem->mkdir( "$path/js/" );
					}
					$wp_filesystem->put_contents( "$path/js/{$cf7_key}.js", $submits['cf7sg_js_file'] );
				} elseif ( isset( $submits['cf7sg_js_file'] ) && $wp_filesystem->is_file( "$path/js/{$cf7_key}.js" ) ) { // delete file.
					if ( ! $wp_filesystem->delete( "$path/js/{$cf7_key}.js" ) ) {
						debug_msg( "CF7SG ADMIN: unable to delete file $path/js/{$cf7_key}.js" );
					}
				}
				// save css file.
				if ( ! empty( $submits['cf7sg_css_file'] ) || ! empty( $submits['cf7sg_prev_css_file'] ) ) {
					...
					if ( ! $wp_filesystem->is_dir( "$path/css/" ) ) {
						$wp_filesystem->mkdir( "$path/css/" );
					}
					$wp_filesystem->put_contents( $path . "/css/{$cf7_key}.css", $submits['cf7sg_css_file'] );
				} elseif ( isset( $submits['cf7sg_css_file'] ) && $wp_filesystem->is_file( "$path/css/{$cf7_key}.css" ) ) { // delete file.
					if ( ! $wp_filesystem->delete( "$path/css/{$cf7_key}.css" ) ) {
		...
		if ( empty( $submits['cf7sg_jstags_comments'] ) ) {
			...
			update_post_meta( $post->ID, '_cf7sg_disable_jstags_comments', 0 );
		}
		/** NB @since 4.3.0 create/update a preview form post for this form */
		$preview_id = get_post_meta( $post->ID, '_cf7sg_form_page', true );
		$content    = '[cf7form cf7key="' . $cf7_key . '"';
		$content   .= isset( $submits['post_lang_choice'] ) ? ' lang="' . sanitize_key( $submits['post_lang_choice'] ) . '"]' : ']';
		$prev_page  = array(
			'post_title'   => sanitize_text_field( $submits['post_title'] ),
		...
		if ( isset( $submits['cf7sg_page_redirect'] ) ) {
			$redirect_to = absint( $submits['cf7sg_page_redirect'] );
			update_post_meta( $post->ID, '_cf7sg_page_redirect', $redirect_to );
			if ( isset( $submits['cache_cf7sg_submit'] ) &&
			isset( $submits['cf7sg_cached_time'] ) &&
			isset( $submits['cf7sg_cached_unit'] ) ) {
				$cache = floatval( $submits['cf7sg_cached_time'] );
				if ( $cache > 0 ) {
					$cache = array( $cache, absint( $submits['cf7sg_cached_unit'] ) );
					update_post_meta( $post->ID, '_cf7sg_cache_redirect_data', $cache );
				}
			} else {
				delete_post_meta( $post->ID, '_cf7sg_cache_redirect_data' );
			}
		}
		if ( empty( $cf7_key ) ) {
			$prev_page['post_content'] = '[contact-form-7 id="' . $post->ID . '"]';
		}
		if ( ! empty( $preview_id ) ) {
		...
		/** NB @since 4.11.5 cf7 plugin validate form */
	...
	public function print_default_js( $form_key ) {
		...
		include_once 'partials/cf7sg-default-js.php';
	}
	/**
	...
	public function duplicate_form_properties( $form_id, $new_form_id ) {
		$properties = apply_filters( 'cf7sg_duplicate_form_properties', self::METAKEYS );
		foreach ( $properties as $field ) {
			$value = get_post_meta( $form_id, $field, true );
			if ( ! empty( $value ) ) {
				update_post_meta( $new_form_id, $field, $value );
			}
		}
	}
	/**
	...
	public function duplicate_cf7_form() {
		...
		if ( isset( $_GET['action'] ) && 'cf7copy' === sanitize_key( $_GET['action'] ) && isset( $_GET['post'] ) ) {
			$pid    = sanitize_key( $_GET['post'] );
			$action = 'wpcf7-copy-contact-form_' . $pid;
			if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['_wpnonce'] ), $action ) ) {
				wp_die( 'Security check error: Try to reload the page and try again.' );
			}
			$action = 'wpcf7-copy-contact-form_' . $pid;
			$form   = wpcf7_contact_form( $pid );
			if ( isset( $form ) ) {
				$new_form = $form->copy();
				$new_form->save();
				self::duplicate_form_properties( $pid, $new_form->id() );
				if ( 'direct' === get_filesystem_method() ) {
					/* you can safely run request_filesystem_credentials() without any issues and don't need to worry about passing in a URL */
					$creds = request_filesystem_credentials( site_url() . '/wp-admin/', '', false, false, array() );
					/* initialize the API */
					if ( WP_Filesystem( $creds ) ) {
						global $wp_filesystem;
						/** NB @since 4.11 duplicate js/css files */
						$new_key = c2p_get_form_key( $new_form->id() );
						$key     = c2p_get_form_key( $form->id() );
						$path    = get_stylesheet_directory();
						$js_file = "$path/js/{$key}.js";
						if ( $wp_filesystem->is_file( $js_file ) ) {
							$wp_filesystem->copy( $js_file, "$path/js/{$new_key}.js" );
						}
						$css_file = "$path/css/{$key}.css";
						if ( $wp_filesystem->is_file( $css_file ) ) {
							$wp_filesystem->copy( $css_file, "$path/css/{$new_key}.css" );
						}
					} // TODO: handle error if failure to get creds.
				} // TODO: handle if only ftp allowed.
	 ...
	 * Hooked on 'cf7sg_kses_allowed_html' as 1st order.
	 *
	 * @since 1.0.0
	 * @param array $allowed array of allowed tags and attributes.
	 * @param mixed $cf7_key form key.
		...
		switch ( $prop ) {
			...
			case 'mail_2':
				...
				if ( class_exists( 'WPCF7_ContactFormTemplate' ) ) {
					$email = WPCF7_ContactFormTemplate::from_email();
				} else {
	...
	public function get_cf7_content() {
		if ( ! isset( $_POST['_wpcf7nonce'] ) ||
		! isset( $_POST['id'] ) ||
		! wp_verify_nonce( sanitize_key( $_POST['_wpcf7nonce'] ), 'wpcf7-save-contact-form_' . sanitize_key( $_POST['id'] ) ) ) {
			echo 'error, nonce failed, try to reload the page.';
			wp_die();
		}
		if ( ! isset( $_POST['cf7_key'] ) ) {
			echo 'error, undefined form key.';
			wp_die();
		}
		$cf7_key   = sanitize_key( $_POST['cf7_key'] );
		$sub_forms = get_posts(
			array(
		...
		if ( ! empty( $sub_forms ) ) {
			...
			echo 'unable to find form ' . esc_attr( $cf7_key );
		}
		wp_die();
	}
	/**
	...
	public function cf7_shortcode_tags() {
		if ( class_exists( 'WPCF7_TagGenerator' ) ) {
			/** NB @since 4.10.0 enable extension of dynamic lists */
	 ...
	 */
	public function benchmark_tag_generator( $contact_form, $args = '' ) {
	...
	public function print_dynamic_list_generator( $tag_id, $dlo, $form, $args ) {
		include plugin_dir_path( __FILE__ ) . 'partials/cf7sg-dynamic-tag-display.php';
	}
	/**
	  ...
	  <strong><?php echo esc_html__( 'Filter imagegrid image sources', 'cf7-grid-layout' ); ?></strong><br />
		...
		echo sprintf(
			wp_kses( /* translators: set of class dynamically populated */
				__(
					'Copy the following <a class="%s" href="javascript:void(0);">filter</a> to your <em>functions.php</em> file.',
					'cf7-grid-layout'
				),
				'cf7sg_filter_taxonomy_images'
			),
			array( // allowed html.
				'a'  => array( 'class', 'href' ),
				'em' => true,
	 ...
	 *
	 * @since 4.0.0
	 * @param Array         $form_classes array of classes to be added to the form wrapper.
	 * @param WPCF7_FormTag $tag cf7 tag object for the form field.
	 * @param String        $form_id cf7 fomr post ID.
	 ...
	 * @param Array $dismissed array of dismissed pointer ids.
	...
	public function reset_meta_cap( $caps ) {
		return array(
			...
			'wpcf7_submit'                         => 'read', /** NB @since 3.2.1 to fix subscribers_only. */
		);
	}
	/**
	 ...
	 * @param Array $post submitted post data.
	 * @return array post data.
	 */
	public function pending_for_review( $data, $post ) {
		if ( $this->cf7_post_type() !== $data['post_type'] ) {
			return $data;
		}
	...
	public function enable_cf7_editor_role() {
		...
		if ( ! empty( $fe ) ) { /** NB @since 3.0.3 in case editor role is deleted */
			foreach ( $caps as $cap ) {
				$fe->add_cap( $cap );
			}
		}
		...
		if ( ! empty( $ad ) ) { /** NB @since 3.0.3 in case admin role is deleted */
			foreach ( $caps as $cap ) {
				$ad->add_cap( $cap );
			}
		}
	...
	public function setup_cf7_mailtags( $mailtags ) {
		...
		if ( ! empty( $fields ) ) {
			...
			foreach ( $toggles as $tgl ) {
				$mailtags[] = 'cf7sg-toggle-' . $tgl;
			}
		}
		// add a general form tag.
		$mailtags[] = 'cf7sg-form-' . c2p_get_form_key( $contact_form->id() );
		/** NB @since 5.0 include form title */
	...
	public function init_notices() {
        ...
        "
			...
			/** NB @since 4.2.0 new sliders tutorial */
	...
	public function admin_notices() {
		...
		if ( ! isset( self::$admin_notice_pages[ $pagenow ] ) ) {
			return;
		}
