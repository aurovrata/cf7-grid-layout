...
(function( $ ) {
  ...
  $(document).ready(function(){
    //setup the mail tags helper code links
    $('span.mailtag.code', '#wpcf7-mail').each(function(){
      let $this = $(this),
        tag = $this.text().replace(/\[(.+)\]/g,'$1'),
        $help = $('<a class="helper" data-tag="'+tag+'" href="javascript:void(0);"><span class="dashicons dashicons-info"></span></a>');
      
      $(this).after($help);
      let helper = mailTagHelper.filter.replace('%s', '<span class="tag-colour">'+tag+'</span>');
      $help.after('<span class="helper">'+helper+', '+mailTagHelper.msg+'</span>');
    });
    $('#wpcf7-mail').on('click','a.helper', async (e)=>{
      e.preventDefault();
      let form = $('#post_name').val();
      if(!form) form = 'my-form';
      let tag    = $(trigger).data('tag'), ftag = tag.replace(/\-/g,'_'),
        addTable = false;
      ...
      let text = "add_filter( 'cf7sg_mailtag_"+tag+"', 'filter_cf7_mailtag_"+ftag+"', 10, 3);\n";
