...
if ( ! class_exists( 'CF7SG_WP_Post_Table' ) ) {
	...
	class CF7SG_WP_Post_Table {
		 ...
		 */
		private static function cf7_post_type() {
		...
		public static function form_id( $key ) {
			...
			if ( isset( self::$forms_key_ids[ $key ] ) ) {
				return self::$forms_key_ids[ $key ];
			} else {
				$form_id = 0;
				$forms   = get_posts(
					array(
		...
		public static function form_key( $id ) {
			...
			if ( false !== ( $key = array_search( $id, self::$forms_key_ids, true ) ) ) {
				return $key;
			} else {
				$key  = null;
				$form = get_post( $id );
		...
		public function modify_cf7_post_type() {
			if ( post_type_exists( self::cf7_post_type() ) ) {
				...
				$wp_post_types[ self::cf7_post_type() ]->show_ui                              = true;
					$wp_post_types[ self::cf7_post_type() ]->labels->add_new                  = __( 'Add a New Form', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->add_new_item             = __( 'Add a New Form', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->edit_item                = __( 'Edit Form', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->new_item                 = __( 'New Form', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->view_item                = __( 'View Form', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->view_items               = __( 'View Forms', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->search_items             = __( 'Search Forms', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->not_found                = __( 'No forms found.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->not_found_in_trash       = __( 'No forms found in Bin.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->attributes               = __( 'Form Attributes', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->insert_into_item         = __( 'Insert into post', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->uploaded_to_this_item    = __( 'Uploaded to this post', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->filter_items_list        = __( 'Filter posts list', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->items_list_navigation    = __( 'Forms list navigation', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->items_list               = __( 'Forms list', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_published           = __( 'Form published.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_published_privately = __( 'Form published privately.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_reverted_to_draft   = __( 'Form reverted to draft.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_trashed             = __( 'Form binned.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_scheduled           = __( 'Form scheduled.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_updated             = __( 'Form updated.', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_link                = __( 'Form Link', 'post-my-contact-form-7' );
					$wp_post_types[ self::cf7_post_type() ]->labels->item_link_description    = __( 'A link to a post.', 'post-my-contact-form-7' );
			}
		}
			...
			switch ( $column ) {
				...
				case 'cf7_key':
					...
					if ( get_post_meta( $post_id, '_cf7sg_managed_form', true ) ) {
						...
						if ( version_compare( $version, CF7SG_VERSION_FORM_UPDATE, '<' ) ) {
							$update = 'cf7sg-update';
						}
					}
					if ( empty( $update ) ) {
		...
		public function shortcode( $atts ) {
			...
			if ( ! empty( $form ) ) {
				...
				if ( ! empty( $fields ) ) { /** @since 4.4.0 */
					add_filter(
						'cf7sg_preview_form_fields',
						function( $pairs, $key ) use ( $fields, $a ) {
							if ( $a['cf7key'] == $key ) {
								return array_merge( $fields, $pairs );
							}
						},
						PHP_INT_MAX,
						2
		...
		(function($){
		  $(document).ready(function(){
			$('tbody#the-list tr').each(function(){
			  ...
			  switch(update){
				case 'cf7sg-update':
				  $tr.find('a.row-title').addClass(update).after('<span class="cf7sg-popup display-none"><?php echo __( 'Please udpate this form, simply edit and update.', 'cf7-grid-layout' ); ?></span>').parent().css('position','relative');
				  break;
				case 'cf7-errors':
				  $tr.find('a.row-title').addClass(update).after('<span class="cf7sg-popup display-none"><?php echo __( 'CF7 plugin has found misconfiguration errors on this form.', 'cf7-grid-layout' ); ?></span>').parent().css('position','relative');
				  break;
			  }
			});
		  });
