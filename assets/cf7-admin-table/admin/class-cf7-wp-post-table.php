...
if ( ! class_exists( 'CF7_WP_Post_Table' ) ) {
	...
	class CF7_WP_Post_Table {
		...
		protected function __construct() {
			...
			$this->version   = '1.3'; // hardcoded and updated when this file changes.
		}
		/**
		...
		public static function form_key( $id ) {
			...
			$key = array_search( $id, self::$forms_key_ids, true );
			if ( false !== $key ) {
				return $key;
			} else {
				$key  = null;
		...
		public static function is_cf7_admin_page() {
			if ( ! isset( $_GET['post_type'] ) || false === strpos( sanitize_key( $_GET['post_type'] ), self::cf7_post_type() ) ) {
				return false;
			} else {
				$screen = get_current_screen();
		...
		public function add_cf7_sub_menu() {
			...
			/** NB @since 4.0.0 helper sub-menu */
			$hook = add_submenu_page(
				'wpcf7',
			...
			switch ( $column ) {
				...
				case 'cf7_key':
					...
					if ( get_post_meta( $post_id, '_cf7sg_managed_form', true ) ) {
						...
						if ( version_compare( $version, '5.0.0', '>=' ) && version_compare( $version, CF7SG_VERSION_FORM_UPDATE, '<' ) ) {
							$update = 'cf7sg-update';
						}
					}
					if ( empty( $update ) ) {
		...
		public function modify_cf7_list_row_actions( $actions, $post ) {
			...
			if ( self::cf7_post_type() === $post->post_type ) {
				...
				if ( current_user_can( 'wpcf7_edit_contact_form', $form->id() ) ) {
					/** NB @since 3.0.0 removed edit/trash as taken care by WP core */
		...
		public function shortcode( $atts ) {
			...
			/** NB @since 4.4.0 enable field values */
			...
			if ( ! empty( $form ) ) {
				$id = $form[0]->ID;
				if ( ! isset( $a['lang'] ) ) { /** NB @since 4.4 allow different lang */
				...
				if ( ! empty( $fields ) ) { /** NB @since 4.4.0 */
					add_filter(
		...
		(function($){
			$(document).ready(function(){
				$('tbody#the-list tr').each(function(){
					...
					switch(update){
					case 'cf7sg-update':
						$tr.find('a.row-title').addClass(update).after('<span class="cf7sg-popup display-none"><?php echo esc_html__( 'Please udpate this form, simply edit and update.', 'cf7-grid-layout' ); ?></span>').parent().css('position','relative');
						break;
					case 'cf7-errors':
						$tr.find('a.row-title').addClass(update).after('<span class="cf7sg-popup display-none"><?php echo esc_html__( 'CF7 plugin has found misconfiguration errors on this form.', 'cf7-grid-layout' ); ?></span>').parent().css('position','relative');
						break;
					}
				});
			});
