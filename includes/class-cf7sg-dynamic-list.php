...
class CF7SG_Dynamic_List {
	...
	public $label;
	/**
	 * Dynamically configure array of dropdown/field styles for the tag builder.
	 *
	 * @since    4.11.0
	 * @access   protected
	...
	/**
	 * Dynamically configure array of extra dropdown/field styles for the tag builder.
	 *
	 * @since    4.11.0
	 * @access   protected
	...
	/**
	 * Dynamically configure array of other attributes for the tag builder.
	 *
	 * @since    4.11.0
	 * @access   protected
	...
	/**
	 * Input option type, either checkbox of radio.
	 *
	 * @since 4.11.0
	 * @access   protected
	...
	/**
	 * Allowed html for wp_kses esc..
	 *
	 * @since 5.0.0
	 * @access   protected
	 * @var Array allowed html for wp_kses escaping.
	 */
	protected $allowed_html = array();
	/**
	 * Static storage of class instances.
	 *
	 * @since    4.10.0
	 * @access   protected
	...
	protected static $instances;
		 ...
		 *
		 * @since 4.10.0
		 * @param String $tag_id field tag id (eg list/checkbox).
		 * @param String $label field tag label.
		 */
	...
	/**
	 * Set the allowed html for wp_kses esc.
	 * You can pass a null value for the default anchor link allowed html.
	 *
	 * @since 5.0.0
	 * @param Array $html_att array of allowed html attributes, see wp_kses() function documentation.
	 */
	public function set_allowed_html( $html_att ) {
		if ( ! isset( $html_att ) ) {
			$html_att = array(
				'a' => array( 'href', 'target' ),
			);
		}
		$this->allowed_html = $html_att;
	}
	/**
	 * Get the allowed html for wp_kses esc.
	 *
	 * @since 5.0.0
	 * @return Array of allowed html attributes, see wp_kses() function documentation.
	 */
	...
	public function get_style_extras( $style ) {
		$extras = array();
		if ( isset( $this->style_extras[ $style ] ) ) {
			$extras = $this->style_extras[ $style ];
		}
		if ( ! is_array( $extras ) ) {
			$extras = array( $extras );
		}
	 ...
	 *
	 * @since 4.10.0
	 * @param String  $taxonomy tax id.
	 * @param Boolean $is_hierarchical hierarchical tax.
	 * @param String  $parent parent term id.
	 * @param String  $level branch level.
	...
	public function get_shortcode_html( $field_name ) {
				...
				if ( ! empty( $posts ) ) {
					 ...
					 *
						...
						*/
						...
						$filter_attributes = apply_filters_deprecated(
							'cf7sg_dynamic_dropdown_option_attributes',
							array(
								array(),
								$post,
								$tag->name,
						...
						if ( is_array( $filter_attributes ) ) {
							if ( isset( $filter_attributes['class'] ) ) {
								if ( ! is_array( $filter_attributes['class'] ) ) {
									$filter_attributes['class'] = array( $filter_attributes['class'] );
								}
								$filter_attributes['class'][] = 'cf7sg-dl';
							} else {
								$filter_attributes['class'] = array( 'cf7sg-dl' );
							}
						} else {
		...
		$attributes = apply_filters_deprecated(
			...
			'4.11.0',
			"cf7sg_{$this->tag_id}_attributes"
		);
		...
		*/
		$default_label = apply_filters_deprecated(
			'cf7sg_dynamic_dropdown_default_value',
			array(
				null,
				$tag->name,
				$cf7_key,
			...
			'4.11.0',
			"cf7sg_{$this->tag_id}_default_value"
		);
		...
		if ( ! empty( $other_attrs ) ) {
			$other_classes = ' cf7sg-dl-' . implode( ' cf7sg-dl-', array_keys( $other_attrs ) );
		}
	...
	protected function get_taxonomy_terms( $source, $branch, $tag, $cf7_key ) {
		...
		if ( ! empty( $branch ) ) {
			$taxonomy_query['parent'] = end( $branch );
		}
		$taxonomy_query = apply_filters_deprecated(
			'cf7sg_dynamic_dropdown_taxonomy_query',
			array(
				$taxonomy_query,
				$tag->name,
				$cf7_key,
			...
			'4.11.0',
			"cf7sg_{$this->tag_id}_taxonomy_query"
		);
		...
		if ( empty( $taxonomy_query ) ) {
			return array(); // nothing more to be done here.
		}
		global $wp_version;
		if ( $wp_version >= 4.5 ) {
			...
			foreach ( $terms as $term ) {
				...
				*/
				...
				if ( is_array( $attributes ) ) {
					...
					$attributes = array( 'class' => 'cf7sg-dl' );
				}
...
if ( ! function_exists( 'cf7sg_create_dynamic_select_tag' ) ) {
	...
	function cf7sg_create_dynamic_select_tag() {
		...
		if ( false === $dl ) {
			...
			$dl->set_allowed_html( null ); // sets default anchor link.
		}
		return $dl;
	}
}
		...
		if ( false === $dl ) {
			...
			$dl->set_styles(
				array(
					...
					'cf7sg-treeview'  => sprintf(
						/* translators: url link */
						__( '<a target="_blank" target="_blank" href="%s">Treeview dropdown</a>', 'cf7-grid-layout' ),
						'https://aurovrata.github.io/hybrid-html-dropdown/examples/#hybrid-dropdown-with-treeview-selection'
					),
					'cf7sg-imagehdd'  => sprintf(
						/* translators: url link */
						__( '<a target="_blank" href="%s">Image dropdown</a>', 'cf7-grid-layout' ),
						'https://aurovrata.github.io/hybrid-html-dropdown/examples/#dropdown-list-with-with-custom-labels-with-images'
					),
					'cf7sg-imagegrid' => sprintf(
						/* translators: url link */
						__( '<a target="_blank" href="%s">Image grid</a>, no dropdown', 'cf7-grid-layout' ),
						'https://aurovrata.github.io/hybrid-html-dropdown/examples/#dropdown-list-with-with-custom-labels-with-images'
					),
				),
				array(
					'cf7sg-imagehdd'  => '(' . __( '<a target="_blank" href="javascript:void(0);">Filter</a> term images', 'cf7-grid-layout' ) . ')',
					'cf7sg-imagegrid' => '(' . __( '<a target="_blank" href="javascript:void(0);">Filter</a> term images', 'cf7-grid-layout' ) . ')',
				)
			);
