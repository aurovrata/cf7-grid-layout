...
class Cf7_Grid_Layout_Public {
	...
	private function localise_script( $params = array(), $css_id = null ) {
		if ( empty( $css_id ) ) {
			...
			if ( isset( $this->localised_data[ $css_id ]['prefill'] ) && isset( $params['prefill'] ) ) { /** NB @since 4.15.0  */
		...
		switch ( $type ) {
			...
			case 'both':
				...
				for ( $idx = 0;$idx < $max_tabs;$idx++ ) { // tables in other tabs.
					...
					} elseif ( $idx > 0 && isset( $_POST[ $table_origin . '_tab-' . $idx ] ) ) {
	...
	public function wpcf7_mail_components( $components, $cf7form, $cf7mail ) {
		...
		/** NB @since 4.9 file validation changes in CF7 v5.4 */
		...
		/** NB @since 2.5.6 fix issue with non-attached files */
		$template = $cf7mail->get( 'attachments' );
		$row      = $tab = null;
		/** NB @since 2.8.1 fix for send pdf */
	...
	public function on_submit_success( $form ) {
		...
		/** NB @since 4.6.0 check if the form is being redirected and data cached */
	...
	public function register_dynamic_list_styles( $airplane, $min ) {
		...
		/** NB @since 3.2.1 use cloudflare for live sites */
	 ...
	 *
	 * @since 4.15.5
	 * @param Array            $results set of reopnse attributes to send back to the form.
	 * @param WPCF7_Submission $submitted cf7 submission object.
	 */
