...
class Cf7_Grid_Layout_Public {
	...
	/**
	 * The processed form submitted $_FILES unslashed.
	 *
	 * @since    2.6.0
	 * @access   private
	 * @var      string    $form_id    The current form id.
	 */
	private $submitted_files = null;

	/**
	 * The processed form submitted $_POST unslashed.
	 *
	 * @since    2.6.0
	 * @access   private
	 * @var      string    $form_id    The current form id.
	 ...
	 */
	private static $array_grid_fields = array();
	 ...
	 */
	private static $array_toggle_fields = array();
	 ...
	 */
	private static $array_tabbed_toggles = array();
	/**
	 * The cf7 array fields.
	 *
	 * @since    2.5.0
	 ...
	 */
	private static $array_toggled_panels = array();
	/**
	 * The cf7 array fields.
	 *
	 * @since    2.6.0
	...
	private function is_set( $field ) {
		if ( ! isset( $this->submitted_data ) ) {
			$this->unslash_posted_data( $form_id );
		}
		return isset( $this->submitted_data[ $field ] );
	}
	/**
	 ...
	 * @param String $field field name.
	 */
	private function has_file( $field ) {
		if ( ! isset( $this->submitted_files ) ) {
			$this->unslash_posted_data();
		}
		return isset( $this->submitted_files[ $field ] );
	}
	/**
	...
	private function get_value( $field ) {
		return $this->is_set( $field ) ? $this->submitted_data[ $field ] : null;
	}
	/**
	 * Get submitted file value.
	 *
	 * @since 5.0.0
	 * @param String $field field name.
	 */
	private function get_file( $field ) {
		return $this->has_file( $field ) ? $this->submitted_files[ $field ] : null;
	}
	/**
	 * Set posted data.
	 *
	 * @since 5.0.0
	 */
	private function unslash_posted_data() {
		$this->submitted_data  = array();
		$this->submitted_files = array();

		if ( isset( $_POST['_cf7sg_nonce'] ) &&
		isset( $_POST['_wpcf7'] ) &&
		wp_verify_nonce( sanitize_key( $_POST['_cf7sg_nonce'] ), 'cf7sg-form-' . sanitize_key( $_POST['_wpcf7'] ) ) ) {
			$this->submitted_data  = wp_unslash( $_POST );
			$this->submitted_files = wp_unslash( $_FILES );
		}
	}
	/**
	 * Retrieve field tabg type for a given field and form id.
	 *
	 * @since 1.0.0
	 ...
	 */
	public static function field_type( $field, $form_id ) {
		...
		if ( isset( self::$array_grid_fields[ $form_id ][ $field ] ) ) {
			$type = self::$array_grid_fields[ $form_id ][ $field ][1];
		}
		return $type;
	}
	...
	protected function is_tabbed( $toggle ) {
		/**
		* Ff the array is not set for this form_id,
	...
	public function register_styles_and_scripts() {
		...
		$ver       = '';
		if ( ! empty( $cf7id ) ) {
			/** NB @since 5.0 check form version */
			...
			/** NB @since 4.15.0 preload the localise data for prefilling */
			add_filter(
				'cf7_2_post_form_values',
				function( $values, $id, $type, $key, $post, $types ) use ( $cf7id, $css_id ) {
					if ( $id != $cf7id ) {
						return $values;
					}
					$this->form_id = $cf7id;
					...
					foreach ( $values as $f => $v ) {
						...
						if ( ! is_null( $toggle ) ) {
							$toggles += array_fill_keys( $toggle, true );
						}
					}
					$this->localise_script(
						array(
							'prefill' => $filter_values,
							'toggles' => $toggles,
						),
						$css_id
					);
		...
		$ui_ver = '1.12.1';
		if ( ! empty( $wp_scripts ) ) {
			$ui_ver = $wp_scripts->query( 'jquery-ui-core' )->ver;
		}
		...
		$min = '';
		if ( ! defined( 'WP_GURUS_DEBUG' ) || ! WP_GURUS_DEBUG ) {
			$min = '.min';
		}
		...
		foreach ( $resources as $class ) {
			switch ( $class ) {
				case 'has-hybriddd': // need hybrid dd styling.
					$dep[] = 'hybriddd-style';
					break;
				case 'has-select2': // need select2 styling.
					$dep[] = 'select2-style';
					break;
				case 'has-nice-select': // need nice-select styling.
					$dep[] = 'jquery-nice-select-css';
					break;
				case 'has-slider': // need glider slider styling.
					$dep[] = 'glider-style';
					break;
				case 'has-toggles':
					break;
		...
		/**
		 * Enabled plugin developers to queue their own styles for the form.
		 *
		 * @since 1.0.0
		 * @param Boolean $airplane true if airplane mode plugin enabled, stop loading scripts.
		 * @param String $min set to '.min' if live site, else empty for debug mode.
		 * @param Array $resources form classes that identify the resoruces required, eg 'has-tabs'.
		 * @param String $cf7key unique form key.
		 * @param String $cf7id form post id.
		 */
		...
		/**
		 * Enabled plugin developers to register their own scripts for the form.
		 *
		 * @since 1.0.0
		 * @param Boolean $airplane true if airplane mode plugin enabled, stop loading scripts.
		 * @param String $min set to '.min' if live site, else empty for debug mode.
	...
	public function add_hidden_fields( $hidden ) {
		...
		/** NB @since 4.4.0 enable rest authentication for logged in users. */
		$hidden['_wpnonce'] = wp_create_nonce( 'wp_rest' );
		/** NB @since 5.0.0 validate data submissions */
		$hidden['_cf7sg_nonce'] = wp_create_nonce( "cf7sg-form-{$form->id()}" );
		return $hidden;
	}
	/**
	 ...
	 *
	 * @since 1.0.0
	 * @param String $output form html string from the CF7 plugin shortcode.
	 * @param String $tag the actual shortcode tag that was used.
	...
	public function cf7_shortcode_request( $output, $tag, $attr ) {
		...
		switch ( true ) {
			case $is_form && empty( $form_version ):
			case $is_form && version_compare( $form_version, '5.0.0', '>=' ) && version_compare( $form_version, CF7SG_VERSION_FORM_UPDATE, '<' ):
		...
		/** NB @since 3.0.0 load scripts only for required classes */
		...
		if ( ! empty( $sub_form_keys ) ) {
			...
			foreach ( $sub_forms as $form ) {
				...
				if ( ! empty( $sub_class ) ) {
					$class = array_unique( array_merge( $class, $sub_class ) );
				}
			}
		}
		// select2.
		...
		/** NB @since 2.6.0 disabled button message*/
		...
		/** NB @since 4.6.0 allow redirect on submit */
		...
		/** NB @since 4.4.0 enable prefilling of form fields, prefill with c2p filter in enqueue fn*/
		$prefill = $this->localise_script();
		if ( isset( $prefill[ $css_id ]['prefill'] ) ) {
			$prefill = $prefill[ $css_id ]['prefill'];
		}
		...
		if ( empty( $prefill ) ) { // fallback on preview values if any.
			$prefill = apply_filters( 'cf7sg_preview_form_fields', array(), $cf7_key ); /** NB @since 4.15.0 */
		}
		...
		if ( $use_grid_js ) {
			...
			if ( ! empty( $sub_forms ) ) {
				$localise['sub_form_nonce'] = wp_create_nonce( "cf7sg-form-{$cf7_id}" );
			}
			$this->localise_script( $localise, $css_id );
			// cf7sg script & style.
		...
		if ( file_exists( $themepath . '/css/' . $cf7_key . '.css' ) ) {
			$dep = array();
			if ( $is_form ) {
				$dep = array( $this->plugin_name );
			}
			wp_enqueue_style( $cf7_key . '-css', $themeuri . '/css/' . $cf7_key . '.css', $dep, $this->version, 'all' );
		}
		if ( file_exists( $themepath . '/js/' . $cf7_key . '.js' ) ) {
			wp_enqueue_script( $cf7_key . '-js', $themeuri . '/js/' . $cf7_key . '.js', array( 'jquery', $this->plugin_name ), $this->version, true );
		...
		/** NB @since 4.4.0 set max-width of forms dynamically */
	...
	/**
	 * Retrieve the form post ID from the slug (unique key)
	 *
	 * @since 4.6.0
	 ...
	 * @param String $css_id form css id attribute.
	...
	private function localise_script( $params = array(), $css_id = null ) {
		if ( empty( $css_id ) ) {
			...
			if ( ! isset( $this->localised_data[ $css_id ] ) ) {
				$this->localised_data[ $css_id ] = array();
			}
			if ( isset( $this->localised_data[ $css_id ]['prefill'] ) && isset( $params[ $css_id ]['prefill'] ) ) { /** NB @since 4.15.0  */
	...
	public function save_grid_fields() {
		...
		! isset( $_POST['id'] ) ||
		! wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'cf7sg-form-' . sanitize_key( $_POST['id'] ) ) ) {
			wp_send_json_error( array( 'message' => 'nonce failed' ) );
			wp_die();
		}
		$post_unslashed = wp_unslash( $_POST );

		if ( isset( $post_unslashed['tabs_fields'] ) && isset( $post_unslashed['table_fields'] ) ) {
			$tabs_fields = json_decode( $post_unslashed['tabs_fields'] );
			if ( empty( $tabs_fields ) ) {
				$tabs_fields = array();
			}
			if ( ! is_array( $tabs_fields ) ) {
				$tabs_fields = array( $tabs_fields );
			}
			...
			$table_fields = json_decode( $post_unslashed['table_fields'] );
		...
		foreach ( $cf7form->scan_form_tags() as $tag ) {
			...
			switch ( $field_type ) {
				...
				default:
					...
					/** NB @since 4.8.1 preserve cf7 data schema for some plugins */
					if ( apply_filters( 'cf7sg_preserve_cf7_data_schema', false ) ) {
						if ( ! empty( $toggle ) && ! $this->is_submitted( $toggle ) ) {
						...
						if ( ! empty( $toggle ) ) {
							...
							$data[ $field_name ] = $this->get_field_value( $field_name, $tag['basetype'], $tag );
						}
					}
					break;
			}
	...
	private function consolidate_grid_submissions( $field_tag, $type, &$data ) {
		$post_unslashed = wp_unslash( $_POST );
		if ( ! isset( $post_unslashed['_cf7sg_nonce'] ) ||
		! isset( $post_unslashed['_wpcf7'] ) ||
		! wp_verify_nonce( sanitize_key( $post_unslashed['_cf7sg_nonce'] ), 'cf7sg-form-' . $post_unslashed['_wpcf7'] ) ) {
			debug_msg( 'CF7SG ERROR: fn consolidate_grid_submissions() is unable to load submitted form' );
			return;
		}

		$cf7_id     = $post_unslashed['_wpcf7'];
		$is_used    = false;
		$field_name = $field_tag['name'];
		$field_type = $field_tag['basetype'];
		$origin     = self::$array_grid_fields[ $cf7_id ][ $field_name ][0];
		...
		switch ( $type ) {
			...
			case 'table':
				...
				$max_fields = isset( $post_unslashed[ $origin ] ) ? $post_unslashed[ $origin ] : 0;
			...
			case 'both':
				...
				$max_tabs     = isset( $post_unslashed[ $tab_origin ] ) ? $post_unslashed[ $tab_origin ] : 0;
				$values['']   = array(); // init multi-dimensional array.

				for ( $idx = 0;$idx < $max_tabs;$idx++ ) { // tables in other tabs.
					$max_fields = 0;
					if ( 0 == $idx && isset( $post_unslashed[ $table_origin ] ) ) {
						$max_fields = $post_unslashed[ $table_origin ];
					} elseif ( $idx > 0 && isset( $post_unslashed[ $table_origin . '_tab-' . $idx ] ) ) {
						$max_fields = $post_unslashed[ $table_origin . '_tab-' . $idx ];
					}
	...
	/**
	 * Extract either a file name or a field value from the submitted $_POST data.
	 *
	 * @since 2.5.0
	 * @param String $field_name field name.
	 * @param String $field_type field type.
	 * @param String $field_tag field cf7 tag base type.
	 * @return mixed array or string field values.
	 */
	private function get_field_value( $field_name, $field_type, $field_tag ) {
		$value = '';
		if ( 'file' === $field_type ) { /** NB @since 4.9.0 handle files in posted data due to CF7 5.4 changes */
			if ( ! $this->has_file( $field_name ) ) {
				return;
			}
			$file = $this->get_file( $field_name );
			...
			$posted = $this->is_set( $field_name ) ? $this->get_value( $field_name ) : '';
			$value  = $posted;

			if ( function_exists( 'wpcf7_form_tag_supports' ) && wpcf7_form_tag_supports( $field_type, 'selectable-values' ) ) {
				$value = (array) $value;
			}
			...
			if ( WPCF7_USE_PIPE && ( $pipes instanceof WPCF7_Pipes ) && ! $pipes->zero() && ! in_array( $field_type, array( 'map', 'dynamic_select' ) ) ) {
				if ( is_array( $posted ) ) {
					...
					$value = $pipes->do_pipe( $posted );
				}
			}

			if ( $field_tag->has_option( 'free_text' ) && $this->is_set( $field_name . '_free_text' ) ) {
				if ( is_array( $value ) ) {
					$v       = array_pop( $value ) . ' ' . sanitize_text_field( $this->get_value( $field_name . '_free_text' ) );
					$value[] = $v;
				} else {
					$value = $value . ' ' . sanitize_text_field( $this->get_value( $field_name . '_free_text' ) );
				}
			}
		}
		return $value;
	 ...
	 **/
	public static function get_grid_fields( $form_id ) {
		if ( isset( self::$array_grid_fields[ $form_id ] ) ) {
		...
		if ( ! empty( $fields ) ) {
			if ( is_array( $fields[ key( $fields ) ] ) ) {
				foreach ( $fields as $table => $table_fields ) {
					$grid_fields += array_fill_keys( $table_fields, array( $table, 'table' ) );
				}
			} else {
				$grid_fields += array_fill_keys( $fields, array( 'cf7-sg-table-000', 'table' ) );
			}
		}
		// tabs.
		$fields = get_post_meta( $form_id, '_cf7sg_grid_tabs_names', true );
		if ( ! empty( $fields ) ) {
			if ( is_array( $fields[ key( $fields ) ] ) ) { /** NB @since 2.4.2 */
				foreach ( $fields as $tab => $tab_fields ) {
					foreach ( $tab_fields as $field ) {
						if ( isset( $grid_fields[ $field ] ) ) {
							$grid_fields[ $field ] = array( $grid_fields[ $field ][0] . ':' . $tab, 'both' );
						} else {
							$grid_fields[ $field ] = array( $tab, 'tab' );
						}
					}
				}
			} else {
				foreach ( $fields as $field ) {
					if ( isset( $grid_fields[ $field ] ) ) {
						$grid_fields[ $field ] = array( $grid_fields[ $field ][0] . ':cf7-sg-tab-000', 'both' );
					} else {
						$grid_fields[ $field ] = array( 'cf7-sg-tab-000', 'tab' );
					}
				}
			}
		}
		...
		if ( $this->is_set( '_cf7sg_toggles' ) ) {
			$toggled_panels = json_decode( $this->get_value( '_cf7sg_toggles' ) );
			if ( ! empty( $toggled_panels ) ) {
				$toggled_panels = get_object_vars( $toggled_panels );
			} else {
				$toggled_panels = array();
			}
		}
		...
		if ( ! empty( $tabtgls ) ) {
			$tabtgls = array_fill_keys( $tabtgls, true );
		} else {
			$tabtgls = array();
		}
	...
	public function validate_required( $result, $tag ) {
		if ( ! $this->is_set( $tag->name ) ) {
			return $result; // not submitted hence disabled.
		}
		$tag = new WPCF7_FormTag( $tag );

		$name  = $tag->name;
		$value = trim( strtr( (string) sanitize_text_field( $this->get_value( $name ) ), "\n", ' ' ) );
	 ...
	 */
	public function validate_swv_schemas( $schema, $form ) {
		// setup the form id.
		...
		foreach ( $tags as $tag ) {
			...
			// the $data array is passed by reference and will be consolidated.
			if ( ! isset( $data[ $tag['name'] ] ) ) { /** NB @todo verify tabbed toggled fields */
		...
		foreach ( $schema->rules() as $r ) { // $rule is a WPCF7_SWV_Rule object.
			$rule = $r->to_array();
			if ( ! isset( $rules[ $rule['field'] ] ) ) {
				$rules[ $rule['field'] ] = array(); // can have multiple rules.
			}
			$rules[ $rule['field'] ][] = $r;
		}
		...
		add_filter(
			'wpcf7_validate',
			function( $result ) use ( $rules, $added_tags, $removed_tags, $valid_tags ) {
	...
	/**
	 * Deep error message duplciator for array fields.
	 *
	 * @since 4.14.0
	 * @param Array $tags of WPCF7_Tags representing the repeated fields.
	 * @param Array $validation validation messages.
	 * @param Int   $d depth level.
	 * @return Array form validations.
	 */
	private function repeat_field_err_messages( $tags, $validation, $d = 1 ) {
		...
		foreach ( $tags as $idx => $tag ) {
			...
			} elseif ( isset( $validation[ $tag['name'] ] ) ) {
	...
	public function filter_wpcf7_validate( $result ) {
		/** NB @since 4.14.0 check cf7 version to maintain backward compatibility */
		if ( defined( 'WPCF7_VERSION' ) && version_compare( WPCF7_VERSION, '5.6', '>=' ) ) {
			return $result;
		}

		/** NB @since 3.3.3 fix for captch field validation */
		$invalids = $result->get_invalid_fields();
		/**
		* NB
		*
		...
		*/
		...
		/** NB @since 3.1.3 allow validation of unvalidated data in custom validation filter */
		$validation = array(); // holds all the validation messages.
		$validated  = array();
		$submission = array();
		/** NB @since 2.1.0 fix issue with Conditional Field plugin.*/
		$data          = $this->remove_hidden_fields_from_conditional_plugin( $data );
		$toggle_status = '';
		if ( $this->is_set( '_cf7sg_toggles' ) ) {
			$toggle_status = json_decode( $this->get_value( '_cf7sg_toggles' ) );
		}

		foreach ( $cf7form->scan_form_tags() as $tag ) {
			/** NB @since 1.9.0 fix issue with Conditional Field plugin.*/
			// validation for non-deprecated plugins (v2.5.0+).
			if ( ! isset( $data[ $tag['name'] ] ) ) {
				continue; // it was removed by some plugin.
			}
			$type = $tag['type'];
			// check to see if this field is an array (table or tab or both).
			...
			switch ( $field_type ) {
				...
				case 'both':
					// the $data array is passed by reference and will be consolidated.
					$values = $data[ $tag['name'] ];
					/** NB @since 3.1.3 track all validations */
							...
							foreach ( $value as $row => $row_value ) {
								...
								if ( 'file' === $tag['basetype'] ) {
									$result = apply_filters(
										"wpcf7_validate_{$sg_field_tag->type}",
										$result,
										$sg_field_tag,
										array(
				...
				default:
					switch ( $type ) {
						case 'captchar': // cannot be called twice, hence see if already invalidated.
							/** NB @since 3.3.3 check if invalidated previously */
	...
	private function filter_validation( $validation, $validated, $submission ) {
		$form_key = '';
		if ( $this->is_set( '_wpcf7_key' ) ) {
			$form_key = $this->get_value( '_wpcf7_key' );
		}
		...
		if ( has_filter( 'cf7sg_validate_submission' ) ) {
		   ...
		   *
			...
			*/
			$validation = apply_filters( 'cf7sg_validate_submission', $validation, $submission, $form_key, $this->get_value( '_wpcf7' ) );
		}
		...
		if ( $result->is_valid() ) {
			...
			* @param int  $form_id  cf7 form post ID.
			*/
			do_action( 'cf7sg_valid_form_submission', $submission, $form_key, $this->get_value( '_wpcf7' ) );
		}
		return $result;
	}
	...
	public function save_select2_custom_options( $post_id, $cf7_key, $post_fields, $post_meta_fields, $submitted_data ) {
		...
		foreach ( $tagged_fields as $field_name => $source ) {
			...
			switch ( $source['source'] ) {
				...
				case 'post':
					...
					apply_filters_deprecated(
						'cf7sg_dynamic_dropdown_post_query',
						array(
							$args,
							$tag->name,
							$cf7_key,
			...
			} elseif ( isset( $post_meta_fields[ $field_name ] ) ) {
	...
	public function filter_table_tab_mail_tag( $replaced, $submitted, $html = false, $mail_tag = null ) {
		...
		switch ( true ) {
			case 0 === strpos( $mail_tag->field_name(), 'cf7sg-toggle-' ):
				...
				if ( isset( $submitted_data[ $toggle ] ) ) {
					$replaced = $submitted_data[ $toggle ];
				}
				break;
			...
			case 'both':
				...
				if ( is_array( $submitted ) ) {
					...
					foreach ( $submitted as $index => $value ) {
						...
						$idx = 0;
						if ( $html ) {
							$replaced .= '<div><span>tab(' . $tab . ')</span>&nbsp;';
						} else {
							$replaced .= 'tab(' . $tab . ') = ';
						}
						if ( is_array( $value ) ) {
							foreach ( $value as $row => $row_value ) {
								...
								if ( is_array( $row_value ) ) {
									$row_value = implode( '|', $row_value );
								}
								if ( $html ) {
									$replaced .= '<div><label>row(' . $idx . '):</label><span>' . $row_value . '</span></div>';
								} else {
									$replaced .= $row_value . ',';
								}
							}
	...
	public function on_submit_success( $form ) {
		...
		if ( ! empty( $prefill ) ) {
			setcookie( '_cf7sg_' . sanitize_text_field( $_POST['_wpcf7_key'] ), json_encode( $prefill ), 0, '/' );
		}
	}
	/**
	...
	public function build_dynamic_checkbox_field( $html, $attrs, $options, $other_attrs, $selected ) {
		...
		if ( isset( $other_attrs['nolimit'] ) ) {
			$attrs['data-limit-selection'] = '-1';
		}
		$attributes  = '';
		$hybrid_data = array();
		switch ( true ) {
		...
		if ( ! empty( $attrs['name'] ) ) {
			$attributes .= ' data-field-name="' . $attrs['name'] . '"';
		}
		if ( isset( $attrs['name'] ) ) {
		...
		foreach ( $attrs as $key => $value ) {
			...
			if ( 'data-maxcheck' == $key ) {
				$key = 'data-limit-selection';
			}
			$attributes .= ' ' . $key . '="' . $value . '"';
		}
		if ( ! $hasId ) {
			$attributes = 'id="dl-' . $attrs['name'] . '" ' . $attributes;
		}
	...
	protected function build_hybrid_list( $options ) {
		...
		foreach ( $options as $value => $details ) {
			...
			$children = array();
			if ( isset( $details[2] ) ) {
				$children = $this->build_hybrid_list( $details[2] );
			}
...
if ( ! function_exists( 'cf7sg_extract_submitted_files' ) ) {
	...
	function cf7sg_extract_submitted_files( array $files ) {
		...
		foreach ( $files as $file ) {
			if ( is_array( $file ) ) {
				$results = array_merge( $results, cf7sg_extract_submitted_files( $file ) );
			} else {
