...
(function( $ ) {
  ...
  $(document).ready( function(){
    ...
    if(cf7Forms.length){
      $('.container.cf7-sg-table', cf7Forms).each(function(){
        ...
        /** @since 4.15.0 prefill, file fields are already omitted */
      ...
      cf7Forms.click('.container', function(e){
        ...
        if( $button.is('div.cf7-sg-table-button a') ){ //----------add a row
          ...
          let $table = $button.prev('.container');
    ...
    if(cf7Forms.length>0){
      ...
      cf7Forms.filter('div.has-toggles form.wpcf7-form').each(function(){
        let $form             = $(this),
            toggled_accordion = $('.cf7sg-collapsible.with-toggle', $form);
        /**
        * @since 1.1.0 track toggle status using toggle ids.
        */
        let toggleStatus             = '', 
            $toggleHiddenStatus      = $('input[name="_cf7sg_toggles"]', $form),
            trackToggle              = false, 
            disableGroupActiveToggle = {};
        ...
        toggled_accordion.each(function(){
          ...
          *@since 1.1.0
          */
          let disableFields = !toggled,
            fid = $section.closest('div.cf7-smart-grid').attr('id');

          $(':input', $section.children('.row')).each(function(){
            let $field = $(this), 
              type     = $field.attr('type'),
              fname    = $field.attr('name').replace('[]','');
            
              if('file' === type) return false; //skip file fields.
            ...
            if(!toggled) $field.prop('disabled', true);
          });
    ...
    $('div.cf7-smart-grid').each(function(){
      ...
      if( !objEmpty( cf7sg[fid],['prefill'] ) ){
        Object.keys(cf7sg[fid].prefill).forEach(function(f){
          let $f = $('[data-name="'+f+'"] :input', $form),
            type = $f.attr('type');

          if(0==$f.length && 'file' !== type){ 
            $f = $(':input[name="'+f+'"]', $form); /* hidden field fix */
          }
    ...
    if(cf7Forms.length > 0){
      //prefill fn will convert niceSelect dropdowns after being filled.
    ...
    if(cf7Forms.length > 0){
      //prefilled select2 will be enabled in the prefill fn.
      //for non cf7 2 post forms, just enable the nice select
      cf7Forms.each(function(){
        let $form = $(this);
    ...
    */
    $('div.cf7-smart-grid.has-toggles div.cf7_2_post form.wpcf7-form').each(function(){
      let $form = $(this), fid = $form.closest('div.cf7-smart-grid').attr('id'),
        nonceID = $form.closest('div.cf7_2_post').attr('id');
      if(nonceID.length>0){
        $form.on(nonceID, function(event){
  ...
  */
  ...
  $.fn.prefillCF7Field = function(val, formID){
    ...
    if( $field.is('.cf7sg-dynamic-dropdown.ui-select:enabled,.wpcf7-form-control.nice-select:enabled') ) {
      $field.niceSelect().trigger('sgNiceSelect');  //eable the nice-select plugin.
    }
    if( $field.is('select.wpcf7-form-control.select2:enabled') ){
      $field.select2($select2.cf7sgSelect2Options()).trigger('sgSelect2');
    }
    if( $field.is('.cf7sg-dynamic_checkbox') ){
      new HybridDropdown(field, $field.cf7sgHybridddOptions());
    }
    return $field;
  }
  ...
  $.fn.cf7sgRemoveRow = function(){
    let $table = $(this);
    if(!$table.is('.container.cf7-sg-table')) return false;
    let rows =  $table.children('.row').not('.cf7-sg-cloned-table-row');
    if(rows.length>1){
      rows.last().remove();
  ...
  $.fn.cf7sgCloneRow = function(initSelect, el){
    ...
    let rowIdx = $table.children( '.row.cf7-sg-table').length - 1, //minus hidden row.
